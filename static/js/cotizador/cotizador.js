$(document).ready( function() {		
	...
	$.ajax({
		...
		'success' : function(obj){
			if(obj['datos_iniciales'].resultado){
				...
				$('#admin_tasa_cambio').val(tarifa_cambio);
				$('#admin_fecha_tasa_cambio').val(tasa_cambio.fecha_tasa);
				$('#admin_costo_por_metro').val(costos_generales.costo_metro);
				$('#admin_indice_utilidad').val(costos_generales.indice_utilidad);
			}
			else {
			...
			if(obj['paneles'].resultado){
				...
				Swal.fire('Ocurrió un error', 'No hay paneles solares registrados o no se pudieron cargar', 'error');
			}
			...
			if(obj['sistemas_monitoreo'].resultado){
				...
				Swal.fire('Ocurrió un error', 'No hay sistemas de monitoreo registrados o no se pudieron cargar', 'error');
			}
			...
			if(obj['inversores'].resultado){
				...
				Swal.fire('Ocurrió un error', 'No hay inversores registrados o no se pudieron cargar', 'error');
			}
			...
			if(obj['optimizadores'].resultado){
				...
				Swal.fire('Ocurrió un error', 'No hay optimizadores registrados o no se pudieron cargar', 'error');
			}
			...
			if(obj['microinversores'].resultado){
				...
				Swal.fire('Ocurrió un error', 'No hay micronversores registrados o no se pudieron cargar', 'error');
			}
			...
			if(obj['tipos_estructura'].resultado){
				...
				Swal.fire('Ocurrió un error', 'No hay tipos de estructura registrados o no se pudieron cargar', 'error');
			}
	...
	$('#cambiar_indice').click(function() {
		if(!cambiando_indice) {
			...
			if(indice_correcto) {
				...
				indice_utilidad = (100-indice_numerico)/100;
	...
	$('#tarifa').change(function() {
		...
		if(datos_consumo.tarifa == "tarifa_pdbt") {
			$('#admin_suministro').val(tarifas_cfe.suministro_comercial);
		}
		else {
			$('#admin_suministro').val(tarifas_cfe.suministro_residencial);
		}
		limpiar_inputs();
	});
	...
	function habilitar_inputs() {
		$('#tipo_interconexion, #tipo_panel, #tipo_estructura_1, #sistema_monitoreo, #instalacion_electrica, #optimizador, #microinversor, #inversor_central, .agregar_estructura').prop("disabled", false); 
	}
	...
	$('#consumo_promedio_kwh').on('input', function() {
		...
		$('#admin_gasto_bimestral').val(datos_consumo.gasto_bimestral);
		$('#admin_gasto_anual').val(datos_consumo.gasto_anual);
		...
		$.ajax({
			...
			'success' : function(obj){
				...
				if(obj['resultado']){
					...
					$('#admin_usd_watt').val(datos_panel.usd_watt);
					$('#admin_watt_panel').val(datos_panel.watts_panel);
					$('#admin_panel_elegido').val(paneles.producto);
					$('#admin_costo_usd_panel').val(datos_panel.usd_panel);
					$('#admin_costo_mxn_panel').val(paneles.costo_mxn);
	...
	$('#num_paneles').change(function() {
		...
		$('#admin_cantidad_panel').val(paneles.num_paneles);
		
		$('#admin_costo_total_panel').val(paneles.costo_total);
	...
	$(document).on('click', '.agregar_estructura', function() {
		...
		if(!$('#fila_estructura_' + id_siguiente).length) {
			...
			var admin_fila_nueva = $('#admin_fila_estructura_' + id_fila).clone();
			$(admin_fila_nueva).prop("id", 'admin_fila_estructura_' + id_siguiente);
			$(admin_fila_nueva).find('label[for="admin_estructura_elegida_' + id_fila + '"]').text("Estructura elegida (" + id_siguiente + ")").prop('for', 'admin_estructura_elegida_' + id_siguiente);;
			$(admin_fila_nueva).find('#admin_estructura_elegida_' + id_fila).prop("id", 'admin_estructura_elegida_' + id_siguiente).prop("name", 'admin_estructura_elegida_' + id_siguiente);
			$(admin_fila_nueva).find('label[for="admin_tipo_estructura_' + id_fila + '"]').prop('for', 'admin_tipo_estructura_' + id_siguiente);;
			$(admin_fila_nueva).find('#admin_tipo_estructura_' + id_fila).prop("id", 'admin_tipo_estructura_' + id_siguiente).prop("name", 'admin_tipo_estructura_' + id_siguiente);
			$(admin_fila_nueva).find('label[for="admin_costo_usd_estructura_' + id_fila + '"]').prop('for', 'admin_costo_usd_estructura_' + id_siguiente);;
			$(admin_fila_nueva).find('#admin_costo_usd_estructura_' + id_fila).prop("id", 'admin_costo_usd_estructura_' + id_siguiente).prop("name", 'admin_costo_usd_estructura_' + id_siguiente);
			$(admin_fila_nueva).find('label[for="admin_costo_mxn_estructura_' + id_fila + '"]').prop('for', 'admin_costo_mxn_estructura_' + id_siguiente);;
			$(admin_fila_nueva).find('#admin_costo_mxn_estructura_' + id_fila).prop("id", 'admin_costo_mxn_estructura_' + id_siguiente).prop("name", 'admin_costo_mxn_estructura_' + id_siguiente);
			$(admin_fila_nueva).find('label[for="admin_cantidad_estructura_' + id_fila + '"]').prop('for', 'admin_cantidad_estructura_' + id_siguiente);;
			$(admin_fila_nueva).find('#admin_cantidad_estructura_' + id_fila).prop("id", 'admin_cantidad_estructura_' + id_siguiente).prop("name", 'admin_cantidad_estructura_' + id_siguiente);
			$(admin_fila_nueva).find('label[for="admin_costo_total_estructura_' + id_fila + '"]').prop('for', 'admin_costo_total_estructura_' + id_siguiente);;
			$(admin_fila_nueva).find('#admin_costo_total_estructura_' + id_fila).prop("id", 'admin_costo_total_estructura_' + id_siguiente).prop("name", 'admin_costo_total_estructura_' + id_siguiente);
			$('#admin_fila_estructura_' + id_fila).after(admin_fila_nueva);
			...
			$('#titulo_estructuras').text("Estructuras (" + id_siguiente + ")");
		}
	});
	...
	$(document).on('click', '.eliminar_estructura', function() {
		...
		if(id_fila > 1) {
			...
			$('#admin_fila_estructura_' + id_fila).remove();
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					$('#admin_estructura_elegida_' + id_fila).val(estructura.codigo);
					$('#admin_tipo_estructura_' + id_fila).val(tipo_estructura);
					$('#admin_costo_usd_estructura_' + id_fila).val(estructura.costo_usd);
					$('#admin_costo_mxn_estructura_' + id_fila).val(estructura.costo_mxn);
					$('#admin_cantidad_estructura_' + id_fila).val(cantidad);
					$('#admin_costo_total_estructura_' + id_fila).val(estructura.costo_total);
	...
	$(document).on('change', '.cantidad_estructura', function() {
		...
		$('#admin_cantidad_estructura_' + id_fila).val(cantidad);
		$('#admin_costo_total_estructura_' + id_fila).val(estructuras[id_fila-1].costo_total);
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					$('#admin_inversor_elegido').val(inversor.producto);
					$('#admin_costo_usd_inversor').val(inversor.costo_usd);
					$('#admin_costo_mxn_inversor').val(inversor.costo_mxn);
					$('#admin_costo_total_inversor').val(inversor.costo_total);
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					$('#admin_optimizador_elegido').val(optimizadores.producto);
					$('#admin_costo_usd_optimizador').val(optimizadores.costo_usd);
					$('#admin_costo_mxn_optimizador').val(optimizadores.costo_mxn);
					$('#admin_cantidad_optimizador').val(optimizadores.cantidad);
					$('#admin_costo_total_optimizador').val(optimizadores.costo_total);
	...
	$('#cantidad_optimizador').change(function() {
		...
		optimizadores.costo_total = round(optimizadores.costo_mxn*cantidad, 2);
		optimizadores.precio_final = round(optimizadores.costo_total/indice_utilidad, 2); 
		$('#admin_cantidad_optimizador').val(cantidad);
		$('#admin_costo_total_optimizador').val(optimizadores.costo_total);
		calcular_cotizacion();
	});
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					$('#admin_microinversor_elegido').val(microinversores.producto);
					$('#admin_costo_usd_microinversor').val(microinversores.costo_usd);
					$('#admin_costo_mxn_microinversor').val(microinversores.costo_mxn);
					$('#admin_cantidad_microinversor').val(microinversores.cantidad);
					$('#admin_costo_total_microinversor').val(microinversores.costo_total);
	...
	$('#cantidad_microinversor').change(function() {
		...
		microinversores.costo_total = round(microinversores.costo_mxn*cantidad, 2);
		microinversores.precio_final = round(microinversores.costo_total/indice_utilidad, 2); 
		$('#admin_cantidad_microinversor').val(cantidad);
		$('#admin_costo_total_microinversor').val(microinversores.costo_total);
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					$('#admin_sistema_monitoreo_elegido').val(sistema_monitoreo.producto);
					$('#admin_costo_usd_sistema_monitoreo').val(sistema_monitoreo.costo_usd);
					$('#admin_costo_mxn_sistema_monitoreo').val(sistema_monitoreo.costo_mxn);
					$('#admin_costo_total_sistema_monitoreo').val(sistema_monitoreo.costo_total);
	...
	$('#instalacion_electrica, #admin_costo_por_metro, #admin_costo_material_electrico, #admin_costo_instalacion').on('change input', function() {
		instalacion_electrica.estado = true;
		instalacion_electrica.metros_tuberia = parseInt($('#instalacion_electrica').val());
		instalacion_electrica.costo_metro = parseFloat($('#admin_costo_por_metro').val());
		instalacion_electrica.material_eletrico = parseInt($('#admin_costo_material_electrico').val());
		instalacion_electrica.instalacion = parseInt($('#admin_costo_instalacion').val());
		...
		$('#admin_costo_tuberia').val(instalacion_electrica.costo_tuberia);
		$('#admin_costo_total_instalacion').val(instalacion_electrica.costo_total);
		
		calcular_cotizacion();
	});
	
	$('#admin_costo_total_instalacion').on('input', function() {
		var costo_instalacion = parseFloat($(this).val());
		if(isNaN(costo_instalacion)) {
			return;
		}
		instalacion_electrica.costo_total = round(costo_instalacion, 2);
		instalacion_electrica.precio_final = round(instalacion_electrica.costo_total/indice_utilidad, 2); 
	...
	function calcular_cotizacion() {
		var subtotal_proveedores = 0;
		var subtotal = 0;
		var subtotal_iva = 0;
		var subtotal_proveedores_iva = 0;
		...
		if(paneles.estado) {
			...
			subtotal_proveedores += paneles.costo_total;
		}
		else {
		...
		if(inversor.estado) {
			...
			subtotal_proveedores += inversor.costo_total;
		}
		else {
		...
		if(optimizadores.estado) {
			...
			subtotal_proveedores += optimizadores.costo_total;
		}
		else {
		...
		if(microinversores.estado) {
			...
			subtotal_proveedores += microinversores.costo_total;
		}
		else {
		...
		if(sistema_monitoreo.estado) {
			...
			subtotal_proveedores += sistema_monitoreo.costo_total;
		}
		else {
		...
		for(var i=0; i<estructuras.length; i++) {
			if(estructuras[i].estado) {
				...
				subtotal_proveedores += estructuras[i].costo_total;
			}
			else {
		...
		if(subtotal > 0 && paneles.num_paneles > 0) {
			...
			totales_cotizacion.costo_proyecto_proveedores = round(subtotal_proveedores, 2); 
			
			// iva del costo del proyecto con proveedores 
			totales_cotizacion.iva_proveedores = subtotal_proveedores_iva;
			
			// costo del proyecto + iva (pago mínimo del cliente)
			totales_cotizacion.costo_proyecto_iva = round(totales_cotizacion.costo_proyecto_proveedores+totales_cotizacion.iva_proveedores, 2);
			
			// precio al cliente (proyecto + utilidad)
			totales_cotizacion.costo_proyecto_utilidad = subtotal;
			
			// utilidad del costo del proyecto
			totales_cotizacion.utilidad = round(totales_cotizacion.costo_proyecto_utilidad-totales_cotizacion.costo_proyecto_proveedores, 2);
			...
			// costo por panel
			totales_cotizacion.costo_panel = round(totales_cotizacion.costo_final/paneles.num_paneles, 2);
			
			// porcentaje de pago mínimo 
			totales_cotizacion.porcentaje_minimo = Math.ceil((totales_cotizacion.costo_proyecto_iva/totales_cotizacion.costo_final)*100);
			
			$('#admin_costo_panel_instalado').val(totales_cotizacion.costo_panel);
			$('#admin_costo_watt_instalado').val(totales_cotizacion.usd_watt);
			$('#admin_costo_proyecto').val(totales_cotizacion.costo_proyecto_proveedores);
			$('#admin_proyecto_iva').val(totales_cotizacion.iva_proveedores);
			$('#admin_proyecto_utilidad').val(totales_cotizacion.utilidad);
			$('#admin_costo_proyecto_iva').val(totales_cotizacion.costo_proyecto_iva);
			$('#admin_costo_proyecto_utilidad').val(totales_cotizacion.costo_proyecto_utilidad);
			$('#admin_costo_proyecto_final').val(totales_cotizacion.costo_final);
			$('#admin_porcentaje_minimo').val(totales_cotizacion.porcentaje_minimo);
