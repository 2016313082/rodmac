...
function cargar_pedidos(){
	var cliente = "";
	$.ajax({
		'url' : base_url + 'pedidos_proveedores/traer_pedidos_inicio',
		'datatype' : 'json',
		'success' : function(obj){
...
function ver_pedidos(){
	/* 
		agregar al array lines, el estatus del producto (se regreso o se acepto)
	*/
	var status_pedido = 0;
	var clase = '';
	var boton = '';
...
function mostrar_ventana(id_pedido){
	...
	swal.fire({
		'icon':'question',
		'title' : 'Desea iniciar la etapa 2?',
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				'url' : base_url + 'pedidos_proveedores/segunda_etapa',
				'type' : 'post',
 ...
 */	$('#enviar_pedido').html('<button class="btn btn-success" onclick="enviar_pedido('+id_pedido+')">Enviar</button>');
	$.ajax({
		'url' : base_url + 'pedidos_proveedores/traer_productos_segunda',
		'type' : 'post',
		'data' : {'id_pedido':id_pedido},
		'datatype' : 'json',
		'success' : function(obj){
			...
			$.each(obj.lines,function(i,elemento){
				...
				var nuevaFila = tabla_productos.row.add([
					...
					elemento.pu_ht,
					elemento.qty,
					'<a class="btn btn-success" href="'+base_url+'pedidos_proveedores/registro_segunda_etapa/'+elemento[0].id+'/'+id_pedido+'" target="_blank"><i class="fa fa-plus" aria-hidden="true"></i></a> '
				]).draw().node();
				$('td', nuevaFila).each(function(index,td){
